...
Apimsg_user_constraint* Apimsg_decoder::test_decode_msg(char* apimsg_body)
{
    char* decode_ptr;
    u_int8_t type;
    int len_tag_len;
...
void Apimsg_decoder::decode_usercons(char* decode_ptr, int total_len, Apimsg_user_constraint* user_cons)
{
	int length=0;
	int starttime=0;
	int endtime=0;
